...
int main() {
    ...
    try {
        ...
        std::cout << "Минимум: " << vec.findMinParallel(numThreads) << std::endl;
        std::cout << "Максимум: " << vec.findMaxParallel(numThreads) << std::endl;
        std::cout << "Сумма: " << vec.findSumParallel(numThreads) << std::endl;
        std::cout << "Среднее: " << vec.findAvgParallel(numThreads) << std::endl;
        std::cout << "Евклидова норма: " << vec.findEuclidParallel(numThreads) << std::endl;
        std::cout << "Манхэттенская норма: " << vec.findManhattanParallel(numThreads) << std::endl;

        // Скалярное произведение с самим собой
        std::cout << "Скалярное произведение с самим собой: "
            << vec.findScalarParallel(vec, numThreads) << std::endl;
    }
